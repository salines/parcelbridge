    ...
    <aside class="column">
        <div class="side-nav">
            ...
            <?php if ($package->isShipped()) : ?>
                <?= $this->Form->postLink(__('Ready for Pickup'), ['action' => 'readyForPickup', $package->id], ['class' => 'side-nav-item']) ?>
                <?= $this->Form->postLink(__('Delivered'), ['action' => 'deliver', $package->id], ['class' => 'side-nav-item']) ?>
            <?php elseif ($package->isReadyForPickup()) : ?>
                <?= $this->Form->postLink(__('Delivered'), ['action' => 'deliver', $package->id], ['class' => 'side-nav-item']) ?>
            <?php endif; ?>
            <?= $this->Form->postLink(__('Delete Package'), ['action' => 'delete', $package->id], ['confirm' => __('Are you sure you want to delete record #{0}?', $package->id), 'class' => 'side-nav-item']) ?>
