...
class Application extends BaseApplication implements AuthenticationServiceProviderInterface, AuthorizationServiceProviderInterface
{
    ...
    public function middleware(MiddlewareQueue $middlewareQueue): MiddlewareQueue
    {
        $middlewareQueue
            ...
            // Identify users before authorization checks run.
            ->add(new AuthenticationMiddleware($this))

            ->add(new AuthorizationMiddleware($this))

            // Enforce request-level portal access through Authorization policies.
    ...
    {
        $isApi = ($request instanceof ServerRequest && $request->getParam('prefix') === 'Api')
            || str_starts_with($request->getUri()->getPath(), '/api');

        $service = new AuthenticationService([
            'unauthenticatedRedirect' => $isApi ? null : '/login',
            'queryParam' => $isApi ? null : 'redirect',
        ]);
        ...
        $service->loadAuthenticator('Authentication.Form', [
            ...
            'loginUrl' => $isApi ? '/api/login' : '/login',
        ]);
