    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        ...
        if (!$fullBaseUrl) {
            throw new InternalErrorException(
                __(
                    'Application base URL is not configured. Set APP_FULL_BASE_URL or configure App.fullBaseUrl.',
                ),
            );
        }
        ...
        if ($configuredHost && $requestHost && strtolower($configuredHost) !== strtolower($requestHost)) {
            throw new BadRequestException(
                __('The request host does not match the configured application host.'),
            );
        }
